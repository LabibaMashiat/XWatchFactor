...
const CategoryOptionDetails = ({ product, setBookingProduct }) => {
  ...
  const handleWishList = (product) => {
    ...
    fetch("https://x-watch-factor-server.vercel.app/products/wishlists", {
      method: "POST",
      headers: {
  ...
  return (
    <div className="card bg-base-100 shadow-2xl w-72">
      <figure>
        <img src={picture} alt="product" className=" h-40" />
      </figure>
      <div className="card-body">
        ...
        <p className="font-semi-bold">
          <u>Resale Price</u> : {resale_price} $
        </p>
        <p className="font-semi-bold">
          <u>Original Price</u> : {original_price} $
        </p>
        <p className="font-semi-bold">
          <u>Years of Use</u> : {years_of_use}
        </p>
        <p className="font-semi-bold">
          <u>Product Description</u> : {product_description}
        </p>
        <div className="flex">
          <p className="font-semi-bold flex">
            <u>Sellers Name</u> : {sellers_name}{" "}
          </p>
          <FaCheckCircle className="bg-green-300 rounded"></FaCheckCircle>
        </div>
        <p className="font-semi-bold">
          <u>Sellers Email</u> : {sellers_email}
        </p>
        <p className="font-semi-bold">
          <u>Sellers Phone</u> : {sellers_phone}
        </p>
        <p className="font-semi-bold">
          <u>Location</u> : {location}
        </p>
        <p className="font-semi-bold">
          <u>Posted Date</u> : {posted_date}
        </p>
        <div>
          {isBuyer && (
            <div>
              ...
              <button
                ...
                className="btn bg-violet-500 p-1 mt-3 rounded w-full"
              >
                Add to WishList <FaBookmark className="mx-3 "></FaBookmark>
              </button>
            </div>
          )}
        </div>
