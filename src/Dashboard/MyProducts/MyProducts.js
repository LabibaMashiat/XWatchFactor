...
const MyProducts = () => {
  ...
  const { data: myProducts = [], refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://x-watch-factor-server.vercel.app/products/${user?.email}`,
        {
          headers: {
  ...
  const handleDeleteProduct = (myProduct) => {
    fetch(
      `https://x-watch-factor-server.vercel.app/allproducts/${myProduct._id}`,
      {
        method: "DELETE",
      }
    )
  ...
  const handleAdvertised = (id) => {
    fetch(`https://x-watch-factor-server.vercel.app/allProducts/${id}`, {
      method: "PUT",
    })
      .then((res) => res.json())
      .then((data) => {
